...
$todayTasksQuery = $conn->prepare("SELECT * FROM tasks WHERE user_id = ? AND DATE(created_at) = CURDATE() AND status != 'Completed'");
if (!$todayTasksQuery) {
    die('Query preparation failed: ' . $conn->error);
}
$todayTasksQuery->bind_param('i', $user_id);
$todayTasksQuery->execute();
$todayTasksResult = $todayTasksQuery->get_result();

$todayDate = date('Y-m-d');
$inProgressQuery = $conn->prepare("SELECT * FROM tasks WHERE user_id = ? AND status = 'In Progress' AND DATE(created_at) != CURDATE()");
if (!$inProgressQuery) {
    die('Query preparation failed: ' . $conn->error);
}
$inProgressQuery->bind_param('i', $user_id);
$inProgressQuery->execute();
$inProgressResult = $inProgressQuery->get_result();


$dueSoonQuery = $conn->prepare("SELECT * FROM tasks WHERE user_id = ? AND due_date <= ? AND status != 'Completed'");
if (!$dueSoonQuery) {
    die('Error in query preparation: ' . $conn->error);
}
$dueSoonQuery->bind_param('is', $user_id, $currentDate);
$dueSoonQuery->execute();
$dueSoonResult = $dueSoonQuery->get_result();
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="task-card p-4" style="background-color:rgb(244, 175, 175); border-radius: 25px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                    <div class="d-flex justify-content-between align-items-center">
                        <h2 style="font-size: 2rem; color: #fff; font-size: 30px; font-weight: bold; text-shadow: 3px 4px 15px rgba(0,0,0,0.3);">
                            Welcome, <?= $_SESSION['username'] ?>!</h2>
    ...
    <div class="container mt-3 p-3" style="background-color:#E8E8E8; border-radius: 20px;">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="section-title new-tasks mb-4 mt-2">New Tasks (Added Today)</div>
            ...
            <!-- In Progress Tasks -->
            <div class="col-md-4">
                <div class="section-title in-progress mb-4 mt-2">In Progress Tasks</div>
                <div class="card-columns">
                    <?php if ($inProgressResult->num_rows > 0): ?>
                        <?php while ($task = $inProgressResult->fetch_assoc()): ?>
                            <div class="card task-card">
                            <div class="card-body" style="background-color: #f9f9f9; border-radius: 10px; padding: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                                    <h5 class="card-title" style="font-size: 1.25rem; color: #333; font-weight: bold;"><?= htmlspecialchars($task['title']) ?></h5>
                                    <p class="card-text" style="font-size: 1rem; color: #666; line-height: 1.5;"><?= htmlspecialchars($task['description']) ?></p>
            ...
            <div class="col-md-4">
                <div class="section-title due-soon mb-4 mt-2" >Tasks Nearing Due Date</div>
                <div class="card-columns">
                    <?php if ($dueSoonResult->num_rows > 0): ?>
                        <?php while ($task = $dueSoonResult->fetch_assoc()): ?>
                            <div class="card task-card">
                            <div class="card-body" style="background-color: #f9f9f9; border-radius: 10px; padding: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                                    <h5 class="card-title" style="font-size: 1.25rem; color: #333; font-weight: bold;"><?= htmlspecialchars($task['title']) ?></h5>
                                    <p class="card-text" style="font-size: 1rem; color: #666; line-height: 1.5;"><?= htmlspecialchars($task['description']) ?></p>
